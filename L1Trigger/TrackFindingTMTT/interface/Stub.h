...
public:
 ...
 Stub(double phi, double r, double z, double bend, int layerid, bool psModule, bool barrel, unsigned int iphi, double alpha) : digitalStub_(), stubWindowSuggest_(), degradeBend_() {
    phi_=phi;
    r_=r;
    z_=z;
    bend_=bend;
...
private:
 ...
 void check2() const {if (digitizeWarningsOn_ && digitizedForHTinput_ && (settings_->firmwareType() >= 1 && settings_->firmwareType() < 99)) throw cms::Exception("Stub: You can't access digitized dphi or rho variables within daisy chain HT firmware!");}
  ...
  void check3() const {if (digitizeWarningsOn_ && digitizedForHTinput_ && (settings_->firmwareType() == 0 || settings_->firmwareType() == 99)) throw cms::Exception("Stub: You can't access the digitized m bin range withing the non-daisy-chain HT firmware!");}
