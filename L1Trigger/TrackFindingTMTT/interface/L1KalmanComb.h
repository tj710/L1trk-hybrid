...
class L1KalmanComb : public TrackFitGeneric{
  ...
  KFTrackletTrack fit(const L1track3D& l1track3D);
  void bookHists();
    protected:
	...
	virtual const kalmanState *kalmanUpdate( unsigned skipped, unsigned layer, const StubCluster* stubCluster, const kalmanState &state);
	void resetStates();
  ...
  virtual std::vector<double> residual(const StubCluster* stubCluster, const std::vector<double> &x, double candQoverPt )const;
	...
	std::vector<const kalmanState *> doKF( const L1track3D &l1track3D, const std::vector<const StubCluster *> &stubClusters);
