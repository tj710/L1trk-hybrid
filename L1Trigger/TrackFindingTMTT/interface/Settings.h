...
namespace TMTT {
 ...
 class Settings {
  ...
  public:
   ...
   bool                 enableMCtruth()           const   {assert(0); return enableMCtruth_;}
   // Enable output histograms & job tracking performance summary (disable to save CPU).
   bool                 enableHistos()            const   {assert(0); return enableHistos_;}

   //=== Cuts on MC truth tracks for tracking efficiency measurements.

   double               genMinPt()                const   {assert(0); return genMinPt_;}
   double               genMaxAbsEta()            const   {assert(0); return genMaxAbsEta_;}
   double               genMaxVertR()             const   {assert(0); return genMaxVertR_;}
   double               genMaxVertZ()             const   {assert(0); return genMaxVertZ_;}
   double               genMaxD0()                const   {assert(0); return genMaxD0_;}
   vector<int>          genPdgIds()               const   {assert(0); return genPdgIds_;}
   // Additional cut on MC truth tracks for algorithmic tracking efficiency measurements.
   unsigned int         genMinStubLayers()        const   {assert(0); return genMinStubLayers_;} // Min. number of layers TP made stub in.
   ...
   bool                 killLowPtStubs()          const   {assert(0); return killLowPtStubs_;}
   // Print stub windows corresponding to KillLowPtStubs, in python cfg format used by CMSSW.
   bool                 printStubWindows()        const   {assert(0); return printStubWindows_;}
   ...
   double               bendResolution()          const   {assert(0); return bendResolution_;}        
   ...
   double               bendResolutionExtra()     const   {assert(0); return bendResolutionExtra_;}        
   // Order stubs by bend in DTC, such that highest Pt stubs are transmitted first.
   bool                 orderStubsByBend()        const   {assert(0); return orderStubsByBend_;}

   //=== Optional stub digitization configuration

   bool                 enableDigitize()          const   {return enableDigitize_;}
   unsigned int         firmwareType()            const   {assert(0); return firmwareType_;}
   //--- Parameters available in MP board.
   unsigned int         phiSectorBits()           const   {assert(0); return phiSectorBits_;}
   unsigned int         phiSBits()                const   {assert(0); return phiSBits_;}
   double               phiSRange()               const   {assert(0); return phiSRange_;}
   unsigned int         rtBits()                  const   {assert(0); return rtBits_;}
   double               rtRange()                 const   {assert(0); return rtRange_;}
   unsigned int         zBits()                   const   {assert(0); return zBits_;}
   double               zRange()                  const   {assert(0); return zRange_;}
   unsigned int         dPhiBits()                const   {assert(0); return dPhiBits_;}
   double               dPhiRange()               const   {assert(0); return dPhiRange_;}
   unsigned int         rhoBits()                 const   {assert(0); return rhoBits_;}
   double               rhoRange()                const   {assert(0); return rhoRange_;}
   //--- Parameters available in GP board (excluding any in common with MP specified above).
   unsigned int         phiOBits()                const   {assert(0); return phiOBits_;}
   double               phiORange()               const   {assert(0); return phiORange_;}
   unsigned int         bendBits()                const   {assert(0); return bendBits_;}
   ...
   bool                 useStubPhi()              const   {assert(0); return useStubPhi_;} // Require stub phi to be consistent with track of Pt > HTArraySpec.HoughMinPt that crosses HT phi axis?
   bool                 useStubPhiTrk()           const   {assert(0); return useStubPhiTrk_;} // Require stub phi0 (or phi65 etc.) as estimated from stub bend, to lie within HT phi axis, allowing tolerance specified below?
   double               assumedPhiTrkRes()        const   {assert(0); return assumedPhiTrkRes_;} // Tolerance in stub phi0 (or phi65) assumed to be this fraction of phi sector width. (N.B. If > 0.5, then stubs can be shared by more than 2 phi sectors).
   bool                 calcPhiTrkRes()           const   {assert(0); return calcPhiTrkRes_;} // If true, tolerance in stub phi0 (or phi65 etc.) will be reduced below AssumedPhiTrkRes if stub bend resolution specified in StubCuts.BendResolution suggests it is safe to do so.
   ...
   vector<double>       etaRegions()              const   {std::vector <double> etaRegions{ -2.4,-2.16,-1.95,-1.7,-1.43,-1.16,-0.89,-0.61,-0.31,0.0,0.31,0.61,0.89,1.16,1.43,1.7,1.95,2.16,2.4};
    return etaRegions_;
   } // Boundaries of eta regions de
   //unsigned int         numEtaRegions()           const   {assert(0); return (etaRegions_.size() - 1);}
   //hacked for hybrid - tjames
   unsigned int         numEtaRegions()           const   {return 18;}
   double               chosenRofZ()              const   {assert(0); return chosenRofZ_;} // Use z of track at this radius for assignment of stubs to phi sectors & also for one of the axes of the r-z HT. 
   double               beamWindowZ()             const   {assert(0); return beamWindowZ_;} // Half-width of window supposed to contain beam-spot in z.
   ...
   bool                 allowOver2EtaSecs()       const   {assert(0); return allowOver2EtaSecs_;} // If True, the code will not throw an error if a stub is assigned to 3 or more eta sectors.
   ...
   unsigned int         houghNbinsPhi()           const   {assert(0); return houghNbinsPhi_;} // Dimension in any track-phi related variable. Not valid if houghNcellsRphi() > 0. (If MiniHTstage = True, this refers to mini cells in whole HT array).
   int                  houghNcellsRphi()         const   {assert(0); return houghNcellsRphi_;} // Required no. of cells in r-phi HT array. If > 0, then parameters HoughNbinsPt and HoughNbinsPhi will be calculated from the constraints that their product should equal HoughNcellsRz and their ratio should make the maximum |gradient|" of stub lines in the HT array equal to 1. If <= 0, then HoughNbinsPt and HoughNbinsPhi will be taken from the values configured above. 
   bool                 enableMerge2x2()          const   {assert(0); return enableMerge2x2_;} // Groups of neighbouring 2x2 cells in HT will be treated as if they are a single large cell. (Also enabled in MiniHTstage = True).
   double               maxPtToMerge2x2()         const   {assert(0); return maxPtToMerge2x2_;} // but only cells with pt < maxPtToMerge2x2() will be merged in this way (irrelevant if enableMerge2x2() = false).
   unsigned int         numSubSecsEta()           const   {assert(0); return numSubSecsEta_;} // Subdivide each sector into this number of subsectors in eta within r-phi HT.
   unsigned int         shape()                   const   {assert(0); return shape_;} // define cell shape (0 square, 1 diamond, 2 hexagon)
   ...
   bool                 miniHTstage()             const   {assert(0); return miniHTstage_;}
   // Number of mini cells along q/Pt & phi axes inside each normal HT cell.
   unsigned int         miniHoughNbinsPt()        const   {assert(0); return miniHoughNbinsPt_;} 
   unsigned int         miniHoughNbinsPhi()       const   {assert(0); return miniHoughNbinsPhi_;} 
   ...
   float                miniHoughMinPt()          const   {assert(0); return miniHoughMinPt_;} 
   // If true, allows tracks found by 1st stage coarse HT to be output if 2nd stage mini HT finds no tracks.
   bool                 miniHoughDontKill()       const   {assert(0); return miniHoughDontKill_;} 
   ...
   float                miniHoughDontKillMinPt()  const   {assert(0); return miniHoughDontKillMinPt_;} 
   ...
   bool                 handleStripsRphiHT()      const   {assert(0); return handleStripsRphiHT_;} 
   ...
   unsigned int         killSomeHTCellsRphi()     const   {assert(0); return killSomeHTCellsRphi_;} 
   ...
   bool                 useBendFilter()           const   {assert(0); return useBendFilter_;} 
   ...
   unsigned int         maxStubsInCell()          const   {assert(0); return maxStubsInCell_;}
   ...
   bool                 busySectorKill()          const   {assert(0); return busySectorKill_;}
   unsigned int         busySectorNumStubs()      const   {assert(0); return busySectorNumStubs_;}
   ...
   vector<unsigned int> busySectorMbinRanges()    const   {assert(0); return busySectorMbinRanges_;}
   ...
   vector<unsigned int> busySectorMbinOrder()     const   {assert(0); return busySectorMbinOrder_;}
   ...
   bool                 busyInputSectorKill()     const   {assert(0); return busyInputSectorKill_;}
   unsigned int         busyInputSectorNumStubs() const   {assert(0); return busyInputSectorNumStubs_;}
   ...
   unsigned int         muxOutputsHT()            const   {assert(0); return muxOutputsHT_;}
   ...
   string               rzFilterName()            const   {assert(0); return rzFilterName_;}
   ...
   unsigned int         dupTrkAlgRphi()           const   {assert(0); return dupTrkAlgRphi_;}
   // Algorithm run on all 3D tracks within each sector after r-z track filter. (Ignored if r-z track filter not run).
   unsigned int         dupTrkAlg3D()             const   {assert(0); return dupTrkAlg3D_;}
   // Algorithm run on tracks after the track helix fit has been done.
   unsigned int         dupTrkAlgFit()            const   {assert(0); return dupTrkAlgFit_;}
   //--- Options used by individual algorithms.
   unsigned int         dupTrkMinCommonHitsLayers() const {assert(0); return dupTrkMinCommonHitsLayers_;}
   ...
   double               generalResidualCut()      const   {assert(0); return generalResidualCut_;}
   double               killingResidualCut()      const   {assert(0); return killingResidualCut_;}

   //--- Additional options for Thomas Schuh's Linear Regression track fitter ---

   // Max allowed iterations.
   unsigned int         maxIterationsLR()                const { assert(0); return maxIterationsLR_;}
   // Internal histograms are filled if it is True
   bool                 LRFillInternalHists()            const { assert(0); return LRFillInternalHists_;}
   bool                 combineResiduals()               const { assert(0); return combineResiduals_; }
   bool                 lineariseStubPosition()          const { assert(0); return lineariseStubPosition_; }
   bool                 checkSectorConsistency()         const { assert(0); return checkSectorConsistency_; }
   bool                 checkHTCellConsistency()         const { assert(0); return checkHTCellConsistency_; }
   unsigned int         minPSLayers()                    const { assert(0); return minPSLayers_; }
   // Digitization
   bool                 digitizeLR()                     const { assert(0); return digitizeLR_; }
   float                PhiPrecision()                   const { assert(0); return PhiPrecision_; }
   float                RPrecision()                     const { assert(0); return RPrecision_; }
   float                ZPrecision()                     const { assert(0); return ZPrecision_; }
   unsigned int         ZSlopeWidth()                    const { assert(0); return ZSlopeWidth_; }
   unsigned int         ZInterceptWidth()                const { assert(0); return ZInterceptWidth_; }
   ...
   bool                digitizeSLR()                     const {assert(0); return digitizeSLR_;}
   /// Number of bits to be used in hardware to compute the division needed to calculate the helix parameters 
   unsigned int        dividerBitsHelix()                const {assert(0); return dividerBitsHelix_;}
   unsigned int        dividerBitsHelixZ()               const {assert(0); return dividerBitsHelixZ_;}
   /// Number of bits to reduce the RPhi helix parameter denominator calculation weight 
   unsigned int        ShiftingBitsDenRPhi()                    const {assert(0); return ShiftingBitsDenRPhi_;}

   /// Number of bits to reduce the RZ helix parameter denominator calculation weight 
   unsigned int        ShiftingBitsDenRZ()                    const {assert(0); return ShiftingBitsDenRZ_;}
   /// Number of bits to reduce the qOverPt parameter numerator calculation weight 
   unsigned int        ShiftingBitsPt()                  const {assert(0); return ShiftingBitsPt_;}
   /// Number of bits to reduce the PhiT parameter numerator calculation weight 
   unsigned int        ShiftingBitsPhi()                  const {assert(0); return ShiftingBitsPhi_;}
   /// Number of bits to reduce the tanLambda parameter calculation weight 
   unsigned int        ShiftingBitsLambda()              const {assert(0); return ShiftingBitsLambda_;}
   /// Number of bits to reduce the tanLambda parameter calculation weight 
   unsigned int        ShiftingBitsZ0()                  const {assert(0); return ShiftingBitsZ0_;}
   /// ChiSquare Cut
   double              slr_chi2cut()                     const {assert(0); return slr_chi2cut_;}
   /// Cut on RPhi Residual (radians)
   double              ResidualCut()                     const {assert(0); return residualCut_;}
   ...
   bool                 kalmanAddBeamConstr()            const { assert(0); return kalmanAddBeamConstr_;}
   ...
   double              deadSimulateFrac()         const   {assert(0); return deadSimulateFrac_;}
   //
   ...
   bool                slr_skipTrackDigi()        const {assert(0); return slr_skipTrackDigi_;}
   unsigned int        slr_oneOver2rBits()        const {assert(0); return slr_oneOver2rBits_;}
   double              slr_oneOver2rRange()       const {assert(0); return slr_oneOver2rRange_;}
   unsigned int        slr_d0Bits()               const {assert(0); return slr_d0Bits_;}
   double              slr_d0Range()              const {assert(0); return slr_d0Range_;}
   unsigned int        slr_phi0Bits()             const {assert(0); return slr_phi0Bits_;}
   double              slr_phi0Range()            const {assert(0); return slr_phi0Range_;}
   unsigned int        slr_z0Bits()               const {assert(0); return slr_z0Bits_;}
   double              slr_z0Range()              const {assert(0); return slr_z0Range_;}
   unsigned int        slr_tanlambdaBits()        const {assert(0); return slr_tanlambdaBits_;}
   double              slr_tanlambdaRange()       const {assert(0); return slr_tanlambdaRange_;}
   unsigned int        slr_chisquaredBits()       const {assert(0); return slr_chisquaredBits_;}
   double              slr_chisquaredRange()      const {assert(0); return slr_chisquaredRange_;}
   // These are used for KF track fitter and for all other track fitters (though are probably not right for other track fitters ...)
   bool                kf_skipTrackDigi()         const {assert(0); return kf_skipTrackDigi_;}
   unsigned int        kf_oneOver2rBits()         const {assert(0); return kf_oneOver2rBits_;}
   double              kf_oneOver2rRange()        const {assert(0); return kf_oneOver2rRange_;}
   unsigned int        kf_d0Bits()                const {assert(0); return kf_d0Bits_;}
   double              kf_d0Range()               const {assert(0); return kf_d0Range_;}
   unsigned int        kf_phi0Bits()              const {assert(0); return kf_phi0Bits_;}
   double              kf_phi0Range()             const {assert(0); return kf_phi0Range_;}
   unsigned int        kf_z0Bits()                const {assert(0); return kf_z0Bits_;}
   double              kf_z0Range()               const {assert(0); return kf_z0Range_;}
   unsigned int        kf_tanlambdaBits()         const {assert(0); return kf_tanlambdaBits_;}
   double              kf_tanlambdaRange()        const {assert(0); return kf_tanlambdaRange_;}
   unsigned int        kf_chisquaredBits()        const {assert(0); return kf_chisquaredBits_;}
   double              kf_chisquaredRange()       const {assert(0); return kf_chisquaredRange_;}
   // Skip track digitisation when fitted is not SimpleLR or KF?
   bool                other_skipTrackDigi()      const {assert(0); return other_skipTrackDigi_;}

   //=== Debug printout & plots

   // Printout level.
   unsigned int         debug()                   const   {assert(0); return debug_;}
   ...
   bool                 resPlotOpt()              const   {assert(0); return resPlotOpt_;}
   // Specify sector for which debug histos for hexagonal HT will be made.
   unsigned int         iPhiPlot()                const   {assert(0); return iPhiPlot_;}
   unsigned int         iEtaPlot()                const   {assert(0); return iEtaPlot_;}
   ...
   bool                 writeOutEdmFile()         const   {assert(0); return writeOutEdmFile_;}
   ...
   // double               invPtToDphi()             const   {assert(0); return (this->getBfield())*(3.0E8/2.0E11);} // B*c/2E11 - converts q/Pt to track angle at some radius from beamline.  
   double               invPtToDphi()             const   {return 3.8*(3.0E8/2.0E11);} // B*c/2E11 - converts q/Pt to track angle at some radius from beamline.  
   ...
   //Hacked to 3.8 for hybrid// tjames
   float                getBfield()               const   {return 3.8;}
