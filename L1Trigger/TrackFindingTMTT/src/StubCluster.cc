...
StubCluster::StubCluster( std::vector<const Stub *> stubs, double SectorPhi, int lres_dr ) 
{
  ...
  tiltedBarrel_ = stubs.at(0)->tiltedBarrel();
    ...
    //std::cout << "endcapRing_ = " << endcapRing_ << ", layerIdReduced_ = " << layerIdReduced_ << ", tiltedBarrel_ = " << tiltedBarrel_ << ", psModule_ = " << psModule_
    //			     << ", moduleTilt_ = " << moduleTilt_ << ", barrel_ = " << barrel_ << ", layerId_ = " << layerId_ <<std::endl; 
    if(nstubs_ == 0 ) return;
