...
//  std::cout << "resids.size(): " << resids.size() << std::endl;
  ...
  for (int i=1;i<numFittingIterations_+1;++i) {
	...
	if ( ! valid ){
	  return L1fittedTrack (getSettings(), l1track3D, stubs_, l1track3D.qOverPt(), 0., l1track3D.phi0(), l1track3D.z0(), l1track3D.tanLambda(), 999999., 4, iPhiSec, iEtaReg, valid);
	}
      }
  ...
  if ( valid4par ){
    return L1fittedTrack(getSettings(), l1track3D, stubs_, tp["qOverPt"], 0, tp["phi0"], tp["z0"], tp["t"], chiSq_, nPar_, iPhiSec, iEtaReg, valid4par);
  }
  else{ 
    return L1fittedTrack (getSettings(), l1track3D, stubs_, l1track3D.qOverPt(), 0., l1track3D.phi0(), l1track3D.z0(), l1track3D.tanLambda(), 999999., 4, iPhiSec, iEtaReg, valid4par);
  }
}
