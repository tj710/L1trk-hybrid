...
void DigitalTrack::makeDigitalTrack() {
  ...
  if (skipTrackDigi_) {
    ...
    // Same again with beam-spot constraint.
    iDigi_oneOver2r_bcon_  = floor(oneOver2r_bcon_orig_*oneOver2rMult_)+1;
    ...
    // Same again with beam-spot constraint.
    oneOver2r_bcon_    = (iDigi_oneOver2r_bcon_)/oneOver2rMult_;
    qOverPt_bcon_      = oneOver2r_bcon_/invPtToDPhi_;
    phi0rel_bcon_      = (iDigi_phi0rel_bcon_)/phi0Mult_;
    phi0_bcon_         = reco::deltaPhi(phi0rel_bcon_, -phiSectorCentre_);
    chisquared_bcon_   = (iDigi_chisquared_bcon_)/chisquaredMult_;
