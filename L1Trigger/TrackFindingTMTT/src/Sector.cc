...
long Sector::forceBitWidth( const float value, const unsigned int nBits) const {

  // slightly hand-waving treatment of 2s complement
  long sign = 1;
  if (value<0) sign = -1;
  long iValue = long(fabs(value));
  long mask =  (long(1)<<nBits)-long(1);
  long result = sign*(iValue & mask);
