...
namespace TMTT {
 ...
 KFTrackletTrack L1KalmanComb::fit(const L1track3D& l1track3D){

  unsigned int iPhiSec=l1track3D.iPhiSec();
  unsigned int  iEtaReg=l1track3D.iEtaReg();
  if (getSettings()->kalmanDebugLevel() >= 1) std::cout << "phi, eta sec = " << iPhiSec << "   " << iEtaReg << std::endl;
  ...
  if (getSettings()->kalmanDebugLevel() >= 1) std::cout << "minStubLayersRed_ = " << minStubLayersRed_ << std::endl;
  //TP
#ifndef HYBRID
  const TP* tpa(0);
  ...
  for( unsigned j_layer=0; j_layer < 16; j_layer++ ){
   ...
   for(unsigned i=0; i < layer_stubs.size(); i++ ){ // Stubs in single layer, ordered by z or r.
    ...
    StubCluster *stbcl = new StubCluster( stubs_for_cls, iPhiSec, 0 );
  ...
  std::vector<const kalmanState *> cands = doKF( l1track3D, stubcls);

  //std::cout << "doKF complete 777" << std::endl;
  //std::cout << "cands.size() = "<< cands.size() << std::endl;
  //return L1fittedTrk for the selected state (if KF produced one it was happy with).

  KFTrackletTrack returnTrk(l1track3D, l1track3D.getStubs(), 0, 0, 0, 0, 0,0,nPar_, iPhiSec, iEtaReg, false);
  if( cands.size() ) {

   //std::cout << "inside cands.size" << std::endl;  
...
//
   ...
   //KFTrackletTrack(l1track3D, cand->stubs(), )
   /*
      38   KFTrackletTrack(const L1track3D& l1track3D, const vector<const Stub*>& stubs,
      39                 float qOverPt, float d0, float phi0, float z0, float tanLambda,
      40                 float chi2, unsigned int nHelixParam,
      41                 unsigned int iPhiSec, unsigned int iEtaReg, bool accepted = true) :
      */
...
*/
   ...
   if (getSettings()->kalmanDebugLevel() >= 1) {
    ...
    if(goodTrack) {
     ...
     int tpin=tpa->index();				
     cout<<"TRACK LOST: eta="<<iEtaReg<<" pt="<<l1track3D.pt()<<" tp="<<tpin<<endl;

     for( auto stubCluster : stubcls ){
      cout<<"    Stub: eta="<<iEtaReg<<" lay_red="<<stubCluster->layerIdReduced()<<" r="<<stubCluster->r()<<" z="<<stubCluster->z()<<"   assoc TPs =";
...
*/
//tp["qOverPt"], tp["d0"], tp["phi0"], tp["z0"], tp["t"]

 
 KFTrackletTrack returnTrk(l1track3D, l1track3D.getStubs(), tp["qOverPt"], tp["d0"], tp["phi0"], tp["z0"], tp["t"],0,nPar_, iPhiSec, iEtaReg, true);

   returnTrk.setInfoKF( 0, numUpdateCalls_ );
//   return returnTrk;

   if (getSettings()->kalmanDebugLevel() >= 1) {
     std::cout << "kfphi " << returnTrk.phi0() << std::endl;
     std::cout << "*****************************" << std::endl;
     std::cout << "Track found" << std::endl;
     std::cout << "*****************************" << std::endl;

     std::cout << "qOverPt = " << tp["qOverPt"] << std::endl;
     std::cout << "phi0 = " << tp["phi0"] << std::endl;
     std::cout << "z0 = " << tp["z0"] << std::endl;
     std::cout << "tanLambda = " << tp["t"] << std::endl;
     std::cout << "*****************************" << std::endl;
   }
   return returnTrk;
  }
 ...
 std::vector<const kalmanState *> L1KalmanComb::doKF( const L1track3D& l1track3D, const std::vector<const StubCluster *> &stubClusters ){
#ifdef RECALC_DEBUG
  ...
  for( unsigned iteration = 0; iteration < maxIterations; iteration++ ){   
   ...
   for(; i_state != prev_states.end(); i_state++ ){ 
    ...
    if (getSettings()->kalmanDebugLevel() >= 1) std::cout << "layer under search = " << layer << std::endl;
    ...
    while ( layerStubs[layer].size() == 0 && layer<7) {
     layer += 1;
     ++nSkippedDeadLayers;
    }
    ...
    if (getSettings()->kalmanDebugLevel() >= 1) std::cout << "nStubs found = " << stubs.size() << std::endl;
    ...
    /*if ( skipped < kalmanMaxSkipLayers ) {
     if ( kalmanDeadLayers.find(layer+1) != kalmanDeadLayers.end()  && layerStubs[layer+1].size() == 0 ) {
      ...
      if (getSettings()->kalmanDebugLevel() >= 1) std::cout << "next layer nStubs found = " << stubs.size() << std::endl;
     }
     }*/

   // if ( skipped < kalmanMaxSkipLayers ) {
      if( layerStubs[layer+2].size() == 0 && layerStubs[layer+1].size() == 0){
      next_stubs = layerStubs[layer+3];
      nSkippedDeadLayers_nextStubs += 2;
    } else if (  layerStubs[layer+1].size() == 0 ) {
      next_stubs = layerStubs[layer+2];
      nSkippedDeadLayers_nextStubs += 1;
     } else{
      next_stubs = layerStubs[layer+1];
   ...
   // }
    ...
    for( unsigned i=0; i < stubs.size()  ; i++ ){
     ...
     const kalmanState * new_state = kalmanUpdate( skipped, layer+1, next_stubCluster, *the_state );
   ...
   //  std::cout << "isGoodState = " << isGoodState(*new_state) << std::endl;   
    ...
    for( unsigned i=0; i < next_stubs.size()  ; i++ ){
     ...
     const kalmanState * new_state = kalmanUpdate( skipped+1+nSkippedDeadLayers_nextStubs, layer+2+nSkippedDeadLayers_nextStubs, next_stubCluster, *the_state );

#ifndef HYBRID			
     if( getSettings()->kalmanFillInternalHists() ) fillStepHists( tpa, iteration, new_state );
#endif

     //std::cout << "isGoodSkippedState = " << isGoodState(*new_state) << std::endl;    
   ...
   if( getSettings()->kalmanFillInternalHists() ) {
    ...
    if( hstubCombMap.find(hname) == hstubCombMap.end() ){
     if (getSettings()->kalmanDebugLevel() >= 1) cout << hname << " does not exist." << endl;
    }
    else{
 ...
 return finished_states;
  ...
  const kalmanState *L1KalmanComb::kalmanUpdate( unsigned skipped, unsigned layer, const StubCluster *stubCluster, const kalmanState &state ){
