...
L1fittedTrack SimpleLR::fit( const L1track3D& l1track3D, unsigned int iPhiSec, unsigned int iEtaReg ) {
  ...
  if(settings_->debug() == 6 and digitize_) cout << std::setw(10) << "First Helix parameters: qOverPt = "<< qOverPt << " ("<< floor(qOverPt*qOverPtMult_) << "), phiT = "<< phiT <<" ("<< floor(phiT*phiTMult_)<<") "<< endl;
  ...
  if(settings_->debug() == 6 and digitize_) {
    cout << "HT mbin "<< int(l1track3D.getCellLocationHT().first) - 16 << " cbin "<< int(l1track3D.getCellLocationHT().second) - 32 << " iPhi "<< iPhiSec << " iEta "<< iEtaReg << endl;
    ...
    cout << std::setw(10) << "Final Helix parameters: qOverPt = "<< qOverPt << " ("<< floor(qOverPt*qOverPtMult_) << "), phiT = "<< phiT <<" ("<< floor(phiT*phiTMult_)<<"), zT = "<< zT << " ("<<floor(zT*z0Mult_)<< "), tanLambda = "<<tanLambda<<" (" << floor(tanLambda*tanLambdaMult_) << ")"<< " z0 "<< z0 <<  endl;
  } else if( settings_->debug() == 6 ){
    cout << std::setw(10) << "Final Helix parameters: qOverPt = "<< qOverPt <<", phi0 = "<< phi0 <<", z0 = "<< z0 << ", tanLambda = "<<tanLambda << endl;
  }
  ...
  if(settings_->debug() == 6 and digitize_) {
    ...
    cout << "HT mbin "<< int(l1track3D.getCellLocationHT().first) - 16 << " cbin "<< int(l1track3D.getCellLocationHT().second) - 32 << " iPhi "<< iPhiSec << " iEta "<< iEtaReg << endl;
    cout << std::setw(10) << "First Helix parameters: qOverPt = "<< fitTrk.qOverPt() << " oneOver2r "<< fitTrk.digitaltrack().oneOver2r() <<" ("<< floor(fitTrk.digitaltrack().oneOver2r()*qOverPtMult_) << "), phi0 = "<< fitTrk.digitaltrack().phi0() <<" ("<< fitTrk.digitaltrack().iDigi_phi0rel() <<"), zT = "<< zT << " ("<<floor(zT*z0Mult_)<< "), tanLambda = "<<tanLambda<<" (" << floor(tanLambda*tanLambdaMult_) << ")"<< endl;
  }
