 ...
 */
 ...
 std::vector<double> KF4ParamsComb::seedx(const L1track3D& l1track3D)const{
   ...
   if (getSettings()->kalmanDebugLevel() >= 1) {
     ...
     std::cout << "initial seed phi0 = " << l1track3D.phi0() << std::endl;
   }
  ...
  if (getSettings()->kalmanDebugLevel() >= 1) {
    std::cout << "state seed" <<std::endl;
    std::cout << "INV2R " << x[INV2R] << std::endl; 
    std::cout << "PHI0" << x[PHI0] << std::endl; 
    std::cout << "Z0 " << x[Z0] << std::endl; 
 ...
 TMatrixD KF4ParamsComb::seedP(const L1track3D& l1track3D)const{
  ...
  //p(Z0,Z0) = 10*5.0 * 5.0; 
  //p(T,T) = 10*0.25 * 0.25 * 4; // IRT: increased by factor 4, as was affecting fit chi2.
 ...
 std::vector<double> KF4ParamsComb::d(const StubCluster* stubCluster )const{
  ...
  meas.resize(2);
  meas[PHI] = wrapRadian( stubCluster->phi() - sectorPhi())+M_PI;
 ...
 bool KF4ParamsComb::isGoodState( const kalmanState &state )const
 {
  ...
  //std::cout << "nStubLayers = "<< nStubLayers << ", z0 = " << z0 << ", pt = " << pt << ", chi2 = " << state.chi2()<< std::endl; 
  // state parameter selections
  if( nStubLayers >= 2 ){

    //if( z0 > 15. ) goodState = false;      

   const double tolerance = (nStubLayers >= 4)  ?  0.09  :  0.4;
