...
void DigitalStub::makeSForTFinput(string SForTF) {
  ...
  if (ranMakeSForTFinput_ != SForTF) {
    ...
    iDigi_R_      = iDigi_Rt_ + round(chosenRofPhi_*rtMult_); 

    //--- Determine floating point variables from digtized numbers.
    r_            = (iDigi_R_)/rtMult_;  
    rt_           = r_ - chosenRofPhi_;

    if (SForTF == "KF4ParamsComb" || SForTF == "KF5ParamsComb" ) {
      // Digitize variables that are exclusive to Sioni's Kalman filter, (which uses same multiplier for r & z).
      iDigi_Z_KF_ = round(float(iDigi_Z_) * (rtMult_/zMult_)); // LinkFormatter.vhd does round().
      // Determine floating point variable from digitized one.
      z_          = (iDigi_Z_KF_)/rtMult_;
    } else {
      // If not using Sioni's KF, then restore z value from HT.
      z_          = (iDigi_Z_)/zMult_; 
    }
...
void DigitalStub::checkInRange() const {
  ...
  if (not (iFirmwareType_ >= 1 && iFirmwareType_ < 99) ) {
    // variables not used for daisy chain firmware.
...
void DigitalStub::checkAccuracy() const {
  ...
  if (iFirmwareType_ >= 1 && iFirmwareType_ < 99) {
    // variables not used for daisy chain firmware.
    TD = 0.;
    TE = 0.;
  }
  ...
  if (nErr < maxErr) {
    if (fabs(TA) > 0.001 || fabs(TB) > 0.3 || (fabs(TC) > 0.2 && iFirmwareType_ != 8) || fabs(TD) > 0.005 || fabs(TE) > 0.005 || fabs(TF) > 0.005 || fabs(TG) > 0.01) {
      nErr++;
      cout<<"WARNING: DigitalStub lost precision: "<<TA<<" "<<TB<<" "<<TC<<" "<<TD<<" "<<TE<<" "<<TF<<" "<<TG<<endl;
    }
  }
}
